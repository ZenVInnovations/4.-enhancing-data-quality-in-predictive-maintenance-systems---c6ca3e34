...
import pandas as pd
import numpy as np
from your_ai_module import DataQualityChecker

# Sample test case for missing values
def test_missing_values():
    test_data = pd.DataFrame({
        'temp': [25, np.nan, 28, 30],
        'vibration': [0.1, 0.2, np.nan, 0.3],
        'pressure': [100, 105, 110, np.nan]
    })
    
    checker = DataQualityChecker()
    results = checker.evaluate(test_data)
    
    assert results['missing_values']['temp'] == 1
    assert results['missing_values']['vibration'] == 1
    assert results['missing_values']['pressure'] == 1
